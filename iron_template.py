...
if platform == 'linux' or platform == 'linux2':
    ...
    raise RunTimeError('Cannot recognize the platform ({})!'.format(platform))
...
# change single slash (/) to double backslash (\\) for Windows

#irontemp = Table.read('templates/irontemplate.ipac', format='ipac')
irontemp = Table.read('{0}{1}templates{1}irontemplate.ipac'.format(package_path, splitter),
                      format='ipac')
